# Express Crypto Payment Gateway

Express example for accepting crypto payments with [Zeno Bank](https://zenobank.io) and the [`@zenobank/sdk`](https://www.npmjs.com/package/@zenobank/sdk).
...
Zeno Bank delivers events to `POST /api/webhooks/zenobank`. The raw request body is preserved with `express.raw` and the signature is verified by `zenoBank.webhooks.verifyWebhook` before the event is processed.
...
Then register the URL in the [Zeno Bank Dashboard](https://dashboard.zenobank.io/developer), e.g. `https://a1b2c3d4.ngrok-free.app/api/webhooks/zenobank`.
...
src/
├── main.js                 # entrypoint
├── app.js                  # createApp(): wires routers + middleware
├── error-handler.js        # ZodError → 400, HttpError → status
├── lib/
...
│   └── logger.js
├── zenobank/
│   ├── zenobank.client.js          # singleton ZenoBankClient
│   └── verify-signature.middleware.js
├── orders/
│   ├── orders.store.js     # in-memory Map
│   ├── orders.dto.js       # zod schema + response mapper
│   ├── orders.service.js
│   └── orders.router.js
└── webhooks/
    └── zenobank.router.js
